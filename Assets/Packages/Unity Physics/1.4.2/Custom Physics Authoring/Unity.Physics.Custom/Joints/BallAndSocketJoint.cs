...
namespace Unity.Physics.Authoring
{
    ...
    public abstract class JointBaker<T> : Baker<T>
        where T : BaseJoint
    {
        ...
        public void CreateJointEntities(uint worldIndex, PhysicsConstrainedBodyPair constrainedBodyPair, NativeArray<PhysicsJoint> joints, NativeList<Entity> newJointEntities)
        {
            ...
            for (int i = 0; i < joints.Length; ++i)
            {
                ...
                {
                    AddComponent(jointEntity, new JointEntityBaking() { Entity = entity });
            ...
            if (multipleJoints)
            {
                ...
                for (int i = 0; i < joints.Length; ++i)
                {
                    DynamicBuffer<PhysicsJointCompanion> companions = AddBuffer<PhysicsJointCompanion>(newJointEntities[i]);
