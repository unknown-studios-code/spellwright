...
namespace Unity.Physics.Authoring
{
    ...
    public static partial class PhysicsShapeExtensions
    {
        ...
        internal static CollisionFilter GetFilter(this PhysicsShapeAuthoring shape)
        {
            ...
            return new CollisionFilter { BelongsTo = shape.BelongsTo.Value, CollidesWith = shape.CollidesWith.Value };
        }
        ...
        public static GameObject FindTopmostEnabledAncestor<T>(GameObject shape, List<T> buffer)
            where T : Component
        {
        ...
        {
            GameObject pb = ColliderExtensions.FindFirstEnabledAncestor(shape, PhysicsShapeExtensions_NonBursted.s_PhysicsBodiesBuffer);
            GameObject rb = ColliderExtensions.FindFirstEnabledAncestor(shape, PhysicsShapeExtensions_NonBursted.s_RigidbodiesBuffer);
        ...
        internal static BoxGeometry BakeToBodySpace(this BoxGeometry box, float4x4 localToWorld, float4x4 shapeToWorld, EulerAngles orientation, bool bakeUniformScale = true)
        {
        ...
        internal static void GetBakedPlaneProperties(this PhysicsShapeAuthoring shape, out float3 vertex0, out float3 vertex1, out float3 vertex2, out float3 vertex3)
        {
            ...
            BakeToBodySpace(center, size, orientation, bakeToShape, out vertex0, out vertex1, out vertex2, out vertex3);
        }
        ...
        public static void GetBakedMeshProperties(this PhysicsShapeAuthoring shape, NativeList<float3> vertices, NativeList<int3> triangles, HashSet<UnityEngine.Mesh> meshAssets = null)
        {
        ...
        public static uint GetStableHash(this ConvexHullGenerationParameters generationParameters, ConvexHullGenerationParameters hashedParameters, float tolerance = k_HashFloatTolerance)
        {
            ...
            return math.cmax(math.abs(differences)) < tolerance ? unchecked((uint)hashedParameters.GetHashCode()) : unchecked((uint)generationParameters.GetHashCode());
        }

        // used to hash an array of points in a way that is robust to imprecision
        public static unsafe uint GetStableHash(this NativeList<float3> points, NativeArray<float3> hashedPoints, float tolerance = k_HashFloatTolerance)
        {
        ...
        public static int GetDeviantAxis(this float3 v)
        {
            ...
            return math.cmax(deviation) == deviation.z ? 2
                : math.cmax(deviation) == deviation.y ? 1
                : 0;
        }
    }
}
