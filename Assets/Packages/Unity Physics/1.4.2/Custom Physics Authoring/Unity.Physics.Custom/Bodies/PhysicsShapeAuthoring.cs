...
namespace Unity.Physics.Authoring
{
    ...
    {
        private const string k_IconPath = "Packages/com.unity.physics/Unity.Physics.Editor/Editor Default Resources/Icons/d_BoxCollider@64.png";
        ...
        [SerializeField]
        private float3 m_PrimitiveCenter;

        [SerializeField]
        private float3 m_PrimitiveSize = new(1f, 1f, 1f);

        [SerializeField]
        private EulerAngles m_PrimitiveOrientation = EulerAngles.Default;

        [SerializeField]
        [ExpandChildren]
        private CylindricalProperties m_Capsule = new()
        {
            Height = 1f,
            Radius = 0.5f,
            Axis = 2,
        ...
        [SerializeField]
        [ExpandChildren]
        private CylindricalProperties m_Cylinder = new()
        {
            Height = 1f,
            Radius = 0.5f,
            Axis = 2,
        ...
        [SerializeField]
        [ExpandChildren]
        private ConvexHullGenerationParameters m_ConvexHullGenerationParameters = ConvexHullGenerationParameters.Default.ToAuthoring();
        ...
        [SerializeField]
        private PhysicsMaterialProperties m_Material = new(true);
        ...
        private void GetCylindricalProperties(CylindricalProperties props, out float3 center, out float height, out float radius, out EulerAngles orientation, bool rebuildOrientation)
        {
            center = m_PrimitiveCenter;
            float3 lookVector = math.mul(m_PrimitiveOrientation, new float3 { [props.Axis] = 1f });
            // use previous axis so forward will prefer up
            float3 upVector = math.mul(m_PrimitiveOrientation, new float3 { [k_NextAxis[k_NextAxis[props.Axis]]] = 1f });
            orientation = m_PrimitiveOrientation;
        ...
        {
            GetCylindricalProperties(m_Capsule, out float3 center, out float height, out float radius, out EulerAngles orientationEuler, m_ShapeType != ShapeType.Capsule);
        ...
        {
            GetCylindricalProperties(m_Cylinder, out float3 center, out float height, out float radius, out orientation, m_ShapeType != ShapeType.Cylinder);
        ...
        {
            orientation = m_PrimitiveOrientation;
            return new SphereGeometry { Center = m_PrimitiveCenter, Radius = m_SphereRadius };
        }
        ...
        private static UnityEngine.Mesh ReusableBakeMesh => s_ReusableBakeMesh ?? (s_ReusableBakeMesh = new UnityEngine.Mesh { hideFlags = HideFlags.HideAndDontSave });
        private static UnityEngine.Mesh s_ReusableBakeMesh;

        public void GetConvexHullProperties(NativeList<float3> pointCloud) => GetConvexHullProperties(pointCloud, true, default, default, default, default);

        internal void GetConvexHullProperties(
            NativeList<float3> pointCloud,
            bool validate,
            NativeList<HashableShapeInputs> inputs,
            NativeList<int> allSkinIndices,
        ...
        )
        {
            ...
            if (m_CustomMesh != null)
            {
                ...
                AppendMeshPropertiesToNativeBuffers(transform.localToWorldMatrix, m_CustomMesh, pointCloud, default, validate, inputs, meshAssets);
            }
            else
            {
                using (var scope = new GetActiveChildrenScope<MeshFilter>(this, transform))
                {
                    foreach (MeshFilter meshFilter in scope.Buffer)
                    {
                        ...
                        {
                            AppendMeshPropertiesToNativeBuffers(meshFilter.transform.localToWorldMatrix, meshFilter.sharedMesh, pointCloud, default, validate, inputs, meshAssets);
                        }
                    }
                }
                ...
                {
                    GetAllSkinnedPointsInHierarchyBelongingToShape(this, skinnedPoints, validate, skinnedInputs, allSkinIndices, allBlendShapeWeights);
                    if (pointCloud.IsCreated)
                    {
        ...
        )
        {
            ...
            using (var scope = new GetActiveChildrenScope<SkinnedMeshRenderer>(shape, shape.transform.root))
            {
                foreach (SkinnedMeshRenderer skin in scope.Buffer)
                {
                    ...
                    if (!skin.enabled || mesh == null || (validate && !mesh.IsValidForConversion(shape.gameObject)) || !scope.IsChildActiveAndBelongsToShape(skin))
                    {
                        continue;
                    }
                    ...
                    int vertexIndex = 0;
                    int weightsOffset = 0;
                    float4x4 shapeFromSkin = math.mul(shape.transform.worldToLocalMatrix, skin.transform.localToWorldMatrix);
                    ...
                    var data = HashableShapeInputs.FromSkinnedMesh(mesh, shapeFromSkin, includedIndices.AsArray(), allIncludedIndices, blendShapeWeights, allBlendShapeWeights);
                    inputs.Add(data);
                }
            }
        ...
        public void GetMeshProperties(NativeList<float3> vertices, NativeList<int3> triangles) => GetMeshProperties(vertices, triangles, true, default);

        internal void GetMeshProperties(NativeList<float3> vertices, NativeList<int3> triangles, bool validate, NativeList<HashableShapeInputs> inputs, HashSet<UnityEngine.Mesh> meshAssets = null)
        {
            ...
            if (m_CustomMesh != null)
            {
                ...
                AppendMeshPropertiesToNativeBuffers(transform.localToWorldMatrix, m_CustomMesh, vertices, triangles, validate, inputs, meshAssets);
            }
            else
            {
                using (var scope = new GetActiveChildrenScope<MeshFilter>(this, transform))
                {
                    foreach (MeshFilter meshFilter in scope.Buffer)
                    {
                        if (scope.IsChildActiveAndBelongsToShape(meshFilter, validate))
                        {
                            AppendMeshPropertiesToNativeBuffers(meshFilter.transform.localToWorldMatrix, meshFilter.sharedMesh, vertices, triangles, validate, inputs, meshAssets);
                        }
                    }
                }
            }
        ...
        )
        {
            ...
            using (UnityEngine.Mesh.MeshDataArray meshData = UnityEditor.MeshUtility.AcquireReadOnlyMeshData(mesh))
#else
        ...
        private void UpdateCylinderAxis() => m_Cylinder.Axis = m_PrimitiveSize.GetDeviantAxis();

        private void Sync(ref CylindricalProperties props)
        {
            props.Height = m_PrimitiveSize[props.Axis];
            props.Radius = 0.5f * math.max(m_PrimitiveSize[k_NextAxis[props.Axis]], m_PrimitiveSize[k_NextAxis[k_NextAxis[props.Axis]]]);
        }
        ...
        public void SetCapsule(CapsuleGeometryAuthoring geometry)
        {
            ...
            float height = math.max(radius * 2f, geometry.Height);
...
#pragma warning restore 618
        ...
        public float4x4 GetShapeToWorldMatrix() => new(Math.DecomposeRigidBodyTransform(transform.localToWorldMatrix));

        // matrix to transform point from object's local transform matrix into shape space
        internal float4x4 GetLocalToShapeMatrix() => math.mul(math.inverse(GetShapeToWorldMatrix()), transform.localToWorldMatrix);
