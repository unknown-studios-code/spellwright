...
{
    internal abstract class TagsDrawer<T> : PropertyDrawer
        where T : ScriptableObject, ITagNames
    {
        private static class Styles
        {
            ...
            public static readonly string MultipleAssetsTooltip = L10n.Tr("Multiple {0} assets found. UI will display labels defined in {1}.");
        ...
        private string[] DefaultOptions => m_DefaultOptions ?? (m_DefaultOptions = Enumerable.Range(0, MaxNumCategories).Select(i => string.Format(DefaultFormatString, i)).ToArray());
        ...
        private string[] GetOptions()
        {
            ...
            m_NamesAssets = guids.Select(AssetDatabase.GUIDToAssetPath).Select(AssetDatabase.LoadAssetAtPath<T>).Where(c => c != null).ToArray();
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (EditorGUI.DropdownButton(controlPosition, EditorGUIUtility.TrTempContent(GetButtonLabel(value, options)), FocusType.Passive, EditorStyles.popup))
            {
