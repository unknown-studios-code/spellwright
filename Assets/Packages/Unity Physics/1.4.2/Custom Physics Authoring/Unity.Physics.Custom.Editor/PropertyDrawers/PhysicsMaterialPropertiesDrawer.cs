...
namespace Unity.Physics.Editor
{
    ...
    internal class PhysicsMaterialPropertiesDrawer : BaseDrawer
    {
        private static class Content
        {
            ...
            public static readonly GUIContent BelongsToLabel = EditorGUIUtility.TrTextContent("Belongs To", "Specifies the categories to which this object belongs.");
            public static readonly GUIContent CollidesWithLabel = EditorGUIUtility.TrTextContent(
                "Collides With",
                "Specifies the categories of objects with which this object will collide, " + "or with which it will raise events if intersecting a trigger."
            );
            public static readonly GUIContent CollisionFilterGroupFoldout = EditorGUIUtility.TrTextContent("Collision Filter");
            public static readonly GUIContent CustomFlagsLabel = EditorGUIUtility.TrTextContent("Custom Tags", "Specify custom tags to read at run-time.");
            public static readonly GUIContent FrictionLabel = EditorGUIUtility.TrTextContent(
                "Friction",
                "Specifies how resistant the body is to motion when sliding along other surfaces, " + "as well as what value should be used when colliding with an object that has a different value."
            );
            public static readonly GUIContent RestitutionLabel = EditorGUIUtility.TrTextContent(
                "Restitution",
                "Specifies how bouncy the object will be when colliding with other surfaces, " + "as well as what value should be used when colliding with an object that has a different value."
            );
        ...
        private Dictionary<string, SerializedObject> m_SerializedTemplates = new();
        ...
        private SerializedProperty GetTemplateValueProperty(SerializedProperty property)
        {
            ...
            if (!m_SerializedTemplates.TryGetValue(key, out serializedTemplate) || serializedTemplate?.targetObject != template)
            {
        ...
        private void FindToggleAndValueProperties(
            ...
            SerializedProperty templateValueProperty,
            string relativePath,
            out SerializedProperty toggle,
        ...
        )
        {
            ...
            value =
                toggle.boolValue || templateValueProperty == null ? relative.FindPropertyRelative("m_Value") : templateValueProperty.FindPropertyRelative(relativePath).FindPropertyRelative("m_Value");
        }
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            FindToggleAndValueProperties(property, templateValueProperty, "m_CollisionResponse", out _, out SerializedProperty collisionResponse);
            // Check if regular collider
            var collisionResponseEnum = (CollisionResponsePolicy)collisionResponse.intValue;
            if (collisionResponseEnum == CollisionResponsePolicy.Collide || collisionResponseEnum == CollisionResponsePolicy.CollideRaiseCollisionEvents)
            {
                height += 2f * (EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing);
            }
        ...
        private static void DisplayOverridableProperty(Rect position, GUIContent label, SerializedProperty toggle, SerializedProperty value, bool templateAssigned)
        {
            if (templateAssigned)
            {
        ...
        protected override void DoGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            FindToggleAndValueProperties(property, templateValue, "m_CollisionResponse", out SerializedProperty collisionResponseDropDown, out SerializedProperty collisionResponse);
            ...
            if (collisionResponseEnum == CollisionResponsePolicy.Collide || collisionResponseEnum == CollisionResponsePolicy.CollideRaiseCollisionEvents)
            {
                FindToggleAndValueProperties(property, templateValue, "m_Friction", out toggle, out SerializedProperty friction);
                position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
                position.height = EditorGUIUtility.singleLineHeight;
                DisplayOverridableProperty(position, Content.FrictionLabel, toggle, friction, templateAssigned);

                FindToggleAndValueProperties(property, templateValue, "m_Restitution", out toggle, out SerializedProperty restitution);
                position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
                position.height = EditorGUIUtility.singleLineHeight;
            ...
            position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
            position.height = EditorGUIUtility.singleLineHeight;
            collisionFilterGroup.isExpanded = EditorGUI.Foldout(position, collisionFilterGroup.isExpanded, Content.CollisionFilterGroupFoldout, true);
            if (collisionFilterGroup.isExpanded)
            {
                ++EditorGUI.indentLevel;

                FindToggleAndValueProperties(property, templateValue, "m_BelongsToCategories", out toggle, out SerializedProperty belongsTo);
                position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
                position.height = EditorGUIUtility.singleLineHeight;
                DisplayOverridableProperty(position, Content.BelongsToLabel, toggle, belongsTo, templateAssigned);

                FindToggleAndValueProperties(property, templateValue, "m_CollidesWithCategories", out toggle, out SerializedProperty collidesWith);
                position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
                position.height = EditorGUIUtility.singleLineHeight;
            ...
            position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
            position.height = EditorGUIUtility.singleLineHeight;
            advancedGroup.isExpanded = EditorGUI.Foldout(position, advancedGroup.isExpanded, Content.AdvancedGroupFoldout, true);
            if (advancedGroup.isExpanded)
            {
                ++EditorGUI.indentLevel;

                FindToggleAndValueProperties(property, templateValue, "m_DetailedStaticMeshCollision", out toggle, out SerializedProperty detailedStaticMeshCollision);
                position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
                position.height = EditorGUIUtility.singleLineHeight;
                ...
                FindToggleAndValueProperties(property, templateValue, "m_CustomMaterialTags", out toggle, out SerializedProperty customFlags);
                position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
                position.height = EditorGUIUtility.singleLineHeight;
