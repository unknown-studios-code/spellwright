...
namespace Unity.Physics.Editor
{
    ...
    internal class PhysicsShapeAuthoringEditor : BaseEditor
    {
        private static class Styles
        {
            ...
            public static readonly string MultipleShapeTypesLabel = L10n.Tr("Multiple shape types in current selection.");
            public static readonly string PreviewGenerationNotification = L10n.Tr("Generating collision geometry preview...");

            private static readonly GUIContent k_FitToRenderMeshesLabel = EditorGUIUtility.TrTextContent("Fit to Enabled Render Meshes");
            ...
            public static readonly GUIContent OrientationLabel = EditorGUIUtility.TrTextContent("Orientation", "Euler orientation in the shape's local space (ZXY order).");
            ...
            public static GUIContent GetFitToRenderMeshesLabel(int numTargets, MessageType status) =>
                status >= MessageType.Warning
                    ? numTargets == 1
                        ? k_FitToRenderMeshesWarningLabelSg
                        : k_FitToRenderMeshesWarningLabelPl
            ...
            public static string GetNoGeometryWarning(int numTargets) => numTargets == 1 ? k_NoGeometryWarning[0] : k_NoGeometryWarning[1];
            ...
            public static string GetNonReadableGeometryWarning(int numTargets) => numTargets == 1 ? k_NonReadableGeometryWarning[0] : k_NonReadableGeometryWarning[1];
            ...
            public static string GetMeshWithSkinnedPointsWarning(int numTargets) => numTargets == 1 ? k_MeshWithSkinnedPointsWarning[0] : k_MeshWithSkinnedPointsWarning[1];
            ...
            public static string GetStaticColliderStatusMessage(int numTargets) => numTargets == 1 ? k_StaticColliderStatusMessage[0] : k_StaticColliderStatusMessage[1];

            public static readonly string BoxCapsuleSuggestion = L10n.Tr(
            ...
            public static readonly string BoxPlaneSuggestion = L10n.Tr($"Target {ShapeType.Box} is flat. Consider using a {ShapeType.Plane} instead.");
            public static readonly string BoxSphereSuggestion = L10n.Tr($"Target {ShapeType.Box} has uniform size and large convex radius. Consider using a {ShapeType.Sphere} instead.");
            public static readonly string CapsuleSphereSuggestion = L10n.Tr($"Target {ShapeType.Capsule}'s diameter is equal to its height. Consider using a {ShapeType.Sphere} instead.");
            public static readonly string CylinderCapsuleSuggestion = L10n.Tr($"Target {ShapeType.Cylinder} has a large convex radius. Consider using a {ShapeType.Capsule} instead.");
            public static readonly string CylinderSphereSuggestion = L10n.Tr(
...
#pragma warning disable 649
        ...
        [AutoPopulate]
        private SerializedProperty m_PrimitiveCenter;

        [AutoPopulate]
        private SerializedProperty m_PrimitiveSize;

        [AutoPopulate]
        private SerializedProperty m_PrimitiveOrientation;

        [AutoPopulate]
        private SerializedProperty m_Capsule;

        [AutoPopulate]
        private SerializedProperty m_Cylinder;

        [AutoPopulate]
        private SerializedProperty m_CylinderSideCount;

        [AutoPopulate]
        private SerializedProperty m_SphereRadius;

        [AutoPopulate]
        private SerializedProperty m_ConvexHullGenerationParameters;
        ...
        [AutoPopulate]
        private SerializedProperty m_ForceUnique;
#pragma warning restore 649
        ...
        [NonSerialized]
        private int m_DraggingControlID;

        [NonSerialized]
        private FitToRenderMeshesDropDown m_DropDown;
        ...
        protected override void OnEnable()
        {
            ...
            m_NumImplicitStatic = targets
                .Cast<PhysicsShapeAuthoring>()
                .Count(shape => shape.GetPrimaryBody() == shape.gameObject && shape.GetComponent<PhysicsBodyAuthoring>() == null && shape.GetComponent<Rigidbody>() == null);
        ...
        private class PreviewMeshData : IDisposable
        {
            ...
            private Dictionary<JobHandle, NativeArray<BlobAssetReference<Collider>>> m_PreviewJobsOutput = new();
            ...
            )
            {
                ...
                switch (shape.ShapeType)
                {
                    case ShapeType.ConvexHull:
                        ...
                        return math.hash(new uint3((uint)shape.ShapeType, currentConvexProperties.GetStableHash(hashedConvexParameters), currentPoints.GetStableHash(hashedPoints)));
                    ...
                    case ShapeType.Mesh:
                        ...
                        return math.hash(
                            new uint3((uint)shape.ShapeType, currentPoints.GetStableHash(hashedPoints), math.hash(triangles.GetUnsafePtr(), UnsafeUtility.SizeOf<int3>() * triangles.Length))
                        );
            ...
            public void SchedulePreviewIfChanged(PhysicsShapeAuthoring shape)
            {
                ...
                {
                    uint hash = GetInputHash(shape, currentPoints, m_HashedPoints, m_HashedConvexParameters, out ConvexHullGenerationParameters currentConvexParameters);
                    if (m_InputHash == hash)
                    {
                        return;
                    }
                ...
                m_MostRecentlyScheduledJob = shape.ShapeType == ShapeType.Mesh ? ScheduleMeshPreview(shape, output) : ScheduleConvexHullPreview(shape, output);
                ...
                if (m_PreviewJobsOutput.Count == 1)
                {
                    ...
                    {
                        SceneViewUtility.DisplayProgressNotification(Styles.PreviewGenerationNotification, () => float.PositiveInfinity);
                    };
                }
            }

            private JobHandle ScheduleConvexHullPreview(PhysicsShapeAuthoring shape, NativeArray<BlobAssetReference<Collider>> output)
            {
                ...
                var pointsArray = new NativeArray<float3>(pointCloud.Length, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
            ...
            private JobHandle ScheduleMeshPreview(PhysicsShapeAuthoring shape, NativeArray<BlobAssetReference<Collider>> output)
            {
                ...
                // copy to NativeArray because NativeList not yet compatible with DeallocateOnJobCompletion
                var pointsArray = new NativeArray<float3>(points.Length, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
                pointsArray.CopyFrom(points.AsArray());
                var triangleArray = new NativeArray<int3>(triangles.Length, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
                        ...
                        else
                        {
                            switch (output[0].Value.Type)
                            {
                                case ColliderType.Convex:
                                    ...
                                    DrawingUtility.GetConvexColliderEdges(ref convex, s_ReusableEdges);
                                    Bounds = convex.CalculateAabb();
                                    break;
                                case ColliderType.Mesh:
                                    ref MeshCollider mesh = ref output[0].As<MeshCollider>();
                                    DrawingUtility.GetMeshColliderEdges(ref mesh, s_ReusableEdges);
        ...
        private Dictionary<PhysicsShapeAuthoring, PreviewMeshData> m_PreviewData = new();
        ...
        private void UpdateGeometryState()
        {
            ...
            foreach (PhysicsShapeAuthoring shape in targets)
            {
                ...
                {
                    PhysicsShapeAuthoring.GetAllSkinnedPointsInHierarchyBelongingToShape(shape, skinnedPoints, false, default, default, default);
        ...
        private static GeometryState GetGeometryState(UnityEngine.Mesh mesh, GameObject host)
        {
            return mesh == null ? GeometryState.NoGeometry
                : !mesh.IsValidForConversion(host) ? GeometryState.NonReadableGeometry
                : GeometryState.Okay;
        }
        ...
        {
            EditorGUI.BeginDisabledGroup((m_GeometryState & GeometryState.NoGeometry) == GeometryState.NoGeometry || EditorUtility.IsPersistent(target));
            Rect rect = EditorGUI.IndentedRect(EditorGUILayout.GetControlRect(false, EditorGUIUtility.singleLineHeight, EditorStyles.miniButton));
        ...
        private List<string> m_GeometryStatusMessages = new();
        private HashSet<string> m_ShapeSuggestions = new();
        private MessageType m_Status;
        private List<string> m_StatusMessages = new(8);
        private MessageType m_MatrixStatus;
        private List<MatrixState> m_MatrixStates = new();
        ...
        private void UpdateStatusMessages()
        {
            ...
            foreach (PhysicsShapeAuthoring shape in targets)
            {
                ...
                switch (shape.ShapeType)
                {
                    ...
                    case ShapeType.Cylinder:
                        ...
                        {
                            m_ShapeSuggestions.Add(math.abs(cylinder.Height - (2f * cylinder.Radius)) < k_Epsilon ? Styles.CylinderSphereSuggestion : Styles.CylinderCapsuleSuggestion);
                        }
                        break;
                }
            }
            ...
            m_MatrixStatus = StatusMessageUtility.GetMatrixStatusMessage(m_MatrixStates, out string matrixStatusMessage);
        ...
        private class FitToRenderMeshesDropDown : EditorWindow
        {
            ...
            public static FitToRenderMeshesDropDown Show(Rect buttonRect, string title, SerializedProperty minimumSkinnedVertexWeight)
            {
                ...
                var size = new Vector2(math.max(buttonRect.width, Styles.WindowWidth), (EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing) * 3f);
            ...
            private void ApplyChanges()
            {
                ...
                foreach (PhysicsShapeAuthoring shape in m_MinimumSkinnedVertexWeight.serializedObject.targetObjects)
                {
                    using (var so = new SerializedObject(shape))
                    {
                        shape.FitToEnabledRenderMeshes(so.FindProperty(m_MinimumSkinnedVertexWeight.propertyPath).floatValue);
                        EditorUtility.SetDirty(shape);
                    }
                }
