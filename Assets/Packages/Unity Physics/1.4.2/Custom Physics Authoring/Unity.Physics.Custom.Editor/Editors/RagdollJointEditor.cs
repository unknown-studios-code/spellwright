...
namespace Unity.Physics.Editor
{
    ...
    {
        private EditorUtilities.AxisEditor m_AxisEditor = new();
        private JointAngularLimitHandle m_LimitHandle = new();
        ...
        private static void DrawCone(float3 point, float3 axis, float angle, Color color)
        {
            ...
            const int res = 16;
            var q = quaternion.AxisAngle(dir, 2.0f * (float)math.PI / res);
            for (int i = 0; i < res; i++)
            {
        ...
        protected virtual void OnSceneGUI()
        {
            ...
            {
                EditorUtilities.EditPivot(ragdoll.worldFromA, ragdoll.worldFromB, ragdoll.AutoSetConnected, ref ragdoll.PositionLocal, ref ragdoll.PositionInConnectedEntity, ragdoll);
            }
            if (ragdoll.EditAxes)
            {
                m_AxisEditor.Update(
                    ragdoll.worldFromA,
                    ragdoll.worldFromB,
                    ragdoll.AutoSetConnected,
                    ragdoll.PositionLocal,
                    ragdoll.PositionInConnectedEntity,
                    ref ragdoll.TwistAxisLocal,
                    ref ragdoll.TwistAxisInConnectedEntity,
                    ref ragdoll.PerpendicularAxisLocal,
                    ref ragdoll.PerpendicularAxisInConnectedEntity,
                    ragdoll
                );
                drawCones = true;
            }
            ...
            {
                EditorUtilities.EditLimits(
                    ragdoll.worldFromA,
                    ragdoll.worldFromB,
                    ragdoll.PositionLocal,
                    ragdoll.TwistAxisLocal,
                    ragdoll.TwistAxisInConnectedEntity,
                    ragdoll.PerpendicularAxisLocal,
                    ragdoll.PerpendicularAxisInConnectedEntity,
                    ref ragdoll.MinTwistAngle,
                    ref ragdoll.MaxTwistAngle,
                    m_LimitHandle,
                    ragdoll
                );
            }
